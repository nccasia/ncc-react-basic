# [NCC React Basic](https://nccasia.github.io/ncc-react-basic/)

## 1. Javascript Necessary Knowledge Base

- [var, let and const](./book/01-javascript-necessary-knowledge-base.md#1-var-let-and-const)
- [Arrow function](./book/01-javascript-necessary-knowledge-base.md#2-arrow-function)
- [Import and Export](./book/01-javascript-necessary-knowledge-base.md#3-import-and-export)
- [Classes, Properties and Methods](./book/01-javascript-necessary-knowledge-base.md#4-classes-properties-and-methods)
- [Spread operators and Rest parameters](./book/01-javascript-necessary-knowledge-base.md#5-spread-operators-and-rest-parameters)
- [Destructuring](./book/01-javascript-necessary-knowledge-base.md#6-destructuring)
- [Reference and Primitive types](./book/01-javascript-necessary-knowledge-base.md#7-reference-and-primitive-types)
- [Array functions: map(), forEach(), filter(), some(), find()...](./book/01-javascript-necessary-knowledge-base.md#8-array-functions)

## 2. React Base Features

- [JSX](https://reactjs.org/docs/introducing-jsx.html), [Element](https://reactjs.org/docs/rendering-elements.html)
- [Component Basic: Class-based and Function Component ](https://reactjs.org/docs/components-and-props.html#function-and-class-components)
- [Prop](https://reactjs.org/docs/components-and-props.html) & [PropTypes](https://reactjs.org/docs/typechecking-with-proptypes.html), [Children property](https://reactjs.org/docs/jsx-in-depth.html#children-in-jsx)
- [State & lifecycle](https://reactjs.org/docs/state-and-lifecycle.html)
- [Stateful and Stateless component](#)
- [Method: Handling Event](https://reactjs.org/docs/handling-events.html)
- [Passing method reference between components](#)
- [How to add Two way binding?](#)
- [Styles component, inline styles](https://reactjs.org/docs/dom-elements.html#style)
- [Conditionals](https://reactjs.org/docs/conditional-rendering.html)
- [Lists rendering](https://reactjs.org/docs/lists-and-keys.html)
- [Form & Form validation](https://reactjs.org/docs/forms.html)

## 3. React Advanced

- [Hook: useState(), useEffect()](#)
- [Http request in React (bonus: using Axios)](#)
- [Routing](#)
- [Context](#)
- [Fragment](#)
- [Refs (React.createRef() and useRef() hook)](#)
- [Higher Order Components (HOC)](#)

## 4. Redux (Optional)

- [Redux data flow](#)
- [State, Reducer, Store](#)
- [Dispatching Action](#)
